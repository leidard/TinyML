...
let rec eval_expr (env : value env) (e : expr) : value =
    ...
    | BinOp (e1, "/", e2) -> 
        ...
        match v2 with
        | VLit (LInt 0) 
        | VLit (LFloat 0.0) -> unexpected_error "division by zero"
        | _ -> binop_math ( / ) ( / ) env e1 e2
    ...
    | BinOp (e1, ">", e2) -> binop_comparison ( > ) ( > ) env e1 e2
    | BinOp (e1, ">=", e2) -> binop_comparison ( >= ) ( >= ) env e1 e2
    | BinOp (e1, "<", e2) -> binop_comparison ( < ) ( < ) env e1 e2
    | BinOp (e1, "<=", e2) -> binop_comparison ( <= ) ( <= ) env e1 e2
    | BinOp (e1, "=", e2) -> binop_comparison ( = ) ( = ) env e1 e2
    ...
    | BinOp (e1, "and", e2) -> binop_logical ( && ) env e1 e2
    | BinOp (e1, "or", e2) -> binop_logical ( || ) env e1 e2

    | UnOp ("not", e) -> unop_logical ( not ) env e

    | UnOp ("-", e) -> unop_math ( - ) env e


    | _ -> unexpected_error "eval_expr: unsupported expression: %s [AST: %A]" (pretty_expr e) e

and binop_math op_int op_float env e1 e2 =
    let v1 = eval_expr env e1
    let v2 = eval_expr env e2
    match v1, v2 with
...
and binop_comparison op_int op_float env e1 e2 =
    let v1 = eval_expr env e1
    let v2 = eval_expr env e2
    match v1, v2 with
...
and binop_logical op env e1 e2 =
    let v1 = eval_expr env e1
    let v2 = eval_expr env e2
    match v1, v2 with
